...
exports.scrap_phones = async (configuration, input_file, output_file) => {
  ...
  const fs = require('fs');
  const { get_phones_google, get_phones_google_nightmare } = require('../google');
  ...
  try {
    ...
    for (let i = 0; i < sirets.length; i++) {
      ...
      process.stdout.write(`#${i + 1}\t [${record.siret}] ${record.name}`);
      /*
      ...
      try {
        ...
        if (res && res.phones) {
          if (Array.isArray(res.phones) && res.phones.length > 0) {
            process.stdout.write(`\t ${res.phones[0]}\n`);
            csv_row += `;${res.phones[0]}`;
          } else if (typeof res.phones === 'string') {
            process.stdout.write(`\t ${res.phones}\n`);
            csv_row += `;${res.phones}`;
          } else {
