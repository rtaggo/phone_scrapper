...
exports.scrap_phones = async (configuration, input_file, output_file) => {
  ...
  const fs = require('fs');
  const { get_phones_google, get_phones_google_nightmare, get_browser, close_browser } = require('../google');
  ...
  try {
    let browser = null;
    if (configuration.headerless_browser === 'puppeteer') {
      console.time('init_browser');
      browser = await get_browser();
      console.timeEnd('init_browser');
    }
    let json_data = await csv({
      delimiter: ';',
    }).fromFile(input_file);
    json_data = json_data.slice(0, 50);
    ...
    for (let i = 0; i < sirets.length; i++) {
      ...
      */
      ...
      try {
        ...
        if (configuration.headerless_browser === 'puppeteer') {
          res = await get_phones_google(browser, search_query);
        } else {
          res = await get_phones_google_nightmare(search_query);
        }
        if (res && res.phones) {
