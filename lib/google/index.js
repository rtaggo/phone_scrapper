...
exports.get_phones_google = async (browser, search_query) => {
  try {
    /*
    const browser = await puppeteer.launch({
      executablePath: chromiumExecutablePath,
    ...
    */
    ...
    try {
      await page.$eval('div[data-attrid="kc:/collection/knowledge_panels/has_phone:phone"] span[class="LrzXr zdqRlf kno-fv', (el) => el.innerText);
    } catch (err) {}
    /*
...
exports.close_browser = async (browser) => {
  try {
    await browser.close();
